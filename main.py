...
# The max numbers of password guesses
LIMIT = 1000000000
...
def common_substrings(input_file):
    ...
    with open(input_file, 'r') as df:
        for line in df:
            sline = line.strip()
            counts += Counter(sline[i:j+1]
                              for j in range(len(sline)) for i in range(j+1))
...
def test():
    ...
    lowercase = ['z', 'j', 'q', 'x', 'k', 'v', 'b', 'p', 'g', 'w', 'y' 'f',
                 'm', 'c', 'u', 'l', 'd', 'h', 'r', 's', 'n', 'i', 'o', 'a', 't', 'e']

    test_file = os.path.dirname(os.path.realpath(
        '__file__')) + "/inputfiles/input_file1.txt"
...
def demo(input_file, test_file):
    ...
    for line in chl:
        ...
        tries = brute_force(len(ans), len(ans), ans,
...
def main():
    ...
    cprint(figlet_format('@EnTaroJankov', font='small'))

    df = input('Please specify a training file number [1-10]: ')
    while (df not in [x+1 for x in range(10)]):
        df = input('Please specify a training file number [1-10]: ')
    train = os.path.dirname(os.path.realpath('__file__')) + \
        "/inputfiles/" + 'input_file' + str(df) + '.txt'
    test = os.path.dirname(os.path.realpath('__file__')) + \
        "/inputfiles/" + 'test_file' + str(df) + '.txt'
    print("Using " + train)
